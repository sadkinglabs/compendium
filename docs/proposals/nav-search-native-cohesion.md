...
- G7 (AvatarPicker): CLOSED 2026-08-15 after the owner's live match concluded - fresh-open capture fully painted (grid art, slots, deck pills), scroll-nudge diff was art-glow noise only. NOT reproducing. Phase 6 now has no open remainders.
...
## 9c. Phase 5 implementation (branch `appbar-consolidation`, build 240 - AWAITING OWNER VISUAL APPROVAL)

`components/AppBar.jsx` (59a9f8d): one chassis, three variants (root 27 h1 / sub 22 h2 / mode banner), `BackButton` = the one 44px gold-circle affordance, static frost band promoted from CollectionSubHeader (Codex's either/or resolved: static). Migrated: App context/detail/add-mode headers, CollectionSubHeader (thin adapter), Collection drill band, Collection list detail (now sticky). Boundaries kept: sheet titles = sheet chassis; AvatarPicker keeps its green-scope header. Forward focus-announce ships on sub bars; back-restore focus deferred pending TalkBack device evidence. Gates green; device screencaps verified: Codex detail (circle back + 22px title - the biggest visual change, replacing centered 14px uppercase + text Back), Sets band (pixel-faithful), drill band, Wishlist detail. **Known tradeoff for owner judgment: long titles truncate at 22px where 15px uppercase used to fit (drill shows "Arthurian Leg…" with the Ring + Select flanking).**
