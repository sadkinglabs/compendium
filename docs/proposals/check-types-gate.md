...
**Status: Rev 5 — implementation-review corrections applied (Codex impl-review: Changes required); awaiting owner ratification before push** · Risk: **Standard**

> **Rev 5 (post-merge implementation review — 3 Major, all accepted).** The gate was implemented and merged locally, then Codex reviewed the diff and required changes: (1) the `ddArming` (transitive) contract typing bypassed the proposal's own "stop and re-scope" checkpoint — now an **explicitly authorized, human-accepted exception** with a documented non-gating rationale; (2) the "no `any`" claim was false — the `any`s are now **tightened to structural types**, with a small **enumerated + justified** residue (JSX content, timer handles); (3) owner approval was **unrecorded at merge** — now recorded, pending the owner's ratification. **Not pushed.** Verification re-passed on every gate. (adds build-time tooling + a baseline gate + JSDoc/ambient annotations in the seven owned files; **zero runtime/logic change** — no device gate, build-time only)
...
- **Green-at-rest cleanup is allowed only in the seven owned files.** A fix that would require touching a transitive file *to clear a diagnostic*, or restructuring *logic* in any file, is **out of scope — stop and re-scope** (`checked-js-pilot.md` criterion 4). The gate never pressures the store layer. (Owned green was in fact reached with owned-file annotations only.)
- **Authorized exception — the `ddArming` contract (Rev 5, Codex impl-review Major 1).** Making the bite-proof's second historical injection (`dd.syncLife('enemy')`) an author-time error requires the *callee's* exported contract to be typed. `ddArming.js` therefore receives narrow **contract** annotations only — `@typedef DdSide`, `@typedef DdApi`, and typed `createDdArming` params/return — **no logic and no internal-diagnostic cleanup**. This is a *deliberate, human-accepted* scope exception (see Approval record), not the forbidden green-cleanup pressure: `ddArming` **stays in the KNOWN-TRANSITIVE allowlist**, so its own diagnostics are still discarded and never gate. Typing its contract only flows type information into the *owned* callers, exactly the "a transitive contract change surfaces as an owned call-site diagnostic" mechanism this policy already sanctions. The distinction maintainers should carry: **contract typing of a transitive boundary to catch owned-caller misuse is permitted; clearing a transitive file's own diagnostics to reach green is not.**
...
**Annotations (JSDoc only; no logic touched):** `players` prop shape and the in-file `VModal`/`MaxLifeModal`/`DiceModal` prop `@typedef`s in `LifeCounter.jsx` (callbacks and primitives structurally typed); `restoreSide` arg shape in `matchLife.js`; the `buildMatchSnapshot`/`readMatchSnapshot` param shapes in `matchSnapshot.js`; `COUNTER_BACK_ORDER` as a readonly tuple in `navBack.js` (fixes the over-promising `@returns`); the `dd` cast recovering the `DdApi` type `useRef` erases.

**Deliberately-`any` fields, enumerated and justified (Rev 5, Codex impl-review Major 2).** The result is **not** "no `any`" - a small, bounded set of fields is intentionally `any`, none of them a cross-runtime bridge (the rejected Capacitor-wide `any`):
- **JSX-content props** - `VModal.{children, actions}` only - a React node has **no nameable type without `@types/react`**, which this gate deliberately does not add (`types: []`). (`title`/`subtitle` are typed `string` and `dice` is `{ type: number, value: number | null }`, per the impl re-review Minor - the earlier "opaque JSX" justification was wrong.) Typing `children`/`actions` `any` does not hide a defect the gate targets (caller/boundary *value* mismatches), which are structurally typed.
- **Opaque platform handles** - `createDdArming`'s injected `setTimeout` return and `clearTimeout` argument are timer handles (`number` in the browser, an object under some runtimes); `any` is honest for a handle only ever round-tripped.

Everything else that was `any` in the merged draft is now structurally typed: `DdApi.phase → DdPhase`, `onChange → (who: DdSide, phase: DdPhase) => void`, the modal callbacks (`onClose: () => void`, `onSet: (v: number) => void`, `setDice: (update: any) => void`), and the modal primitives (`open`/`rotated`/`value`/`who`).
...
| Human (approver) | **Approved** the three owner decisions (scoped gate + owned-file cleanup; ratify declining app-wide/`strict`; `typescript` exact-pin devDep + `check:types` baseline) via interactive approval, and **chose the 3/3 bite-proof** — thereby accepting the narrow `ddArming` contract-typing scope exception | 2026-07-18 |
| Codex (reviewer) | **Implementation review: Changes required** — 3 Major (transitive `ddArming` typing bypassed the re-scope checkpoint; "no `any`" contradicted; owner approval unrecorded) · wrapper fail-closed + `dd` cast confirmed sound; all gates re-passed | 2026-07-18 |
| Claude Code (author) | **Rev 5** — tightened the `any`s to structural types (kept only enumerated JSX/timer-handle `any`); authorized + explained the `ddArming` contract exception (non-gating); recorded the owner decisions; corrected the "no `any`" wording. Awaiting owner ratification of the recorded approval before push. | 2026-07-18 |
