...
- **JSX-content props** - `VModal.{title, subtitle, children, actions}`, `DiceModal.dice.value` - a React node / opaque display value has **no nameable type without `@types/react`**, which this gate deliberately does not add (`types: []`). Typing them `any` does not hide a defect the gate targets (caller/boundary *value* mismatches), which are structurally typed.
