...
The provisional/confirmed contract is only prose until a component *owns* confirmed-vs-pending state. It will. New pure module **`src/pillars/ownedStepController.js`** (sibling to `collectionGoalDrain.js`), consumed via the binding **`useOwnedLedger`** (`OwnedControl.jsx`). **Boundary note (Codex):** today `Collection.jsx:stepSet` writes the repository directly; **Phase 2 consolidates every owned stepper (`LedgerRow`, `BinderTile`, `CollectionCardSheet`) to route through `useOwnedLedger`**, making that hook the single consumer boundary — no stepper touching the repository directly. It *will* route through it; it does not yet.
...
- **Reconcile only when the full row chain drains (Codex).** A per-write completion **decrements `pendingCount`** and, on failure, **records the failure but does NOT clear provisional state mid-chain** (later queued taps are still in flight — clearing early would drop them from the display). **Only when `pendingCount` reaches 0** does the controller perform **one** authoritative `read`, replace `confirmedQty`, clear all provisional state, and `notify` **once** if any write in the chain failed.
- **On failure (chain settled):** the single authoritative `read` restores `confirmedQty` to the true stored value, and `notify` emits a visible **"Couldn't save; count restored"** (once per drained chain, not per failed write).
...
- **`ownedStepController` tests (§4.1, pure — injected `read`/`write`/`notify`/`isAlive`, no DB/DOM):** (a) displayed qty = `max(0, confirmedQty + pendingDelta)` through interleaved taps; (b) a rejected `write` restores `confirmedQty` from the authoritative `read` and emits the visible error; (c) after `isAlive`→false (unmount) the write still fires but **no state is applied**; (d) on drain, `confirmedQty` is replaced from the authoritative read and settled provisional state clears; (e) **mid-chain failure** — an early write fails while later taps are still queued: provisional state is **not** cleared mid-chain, and exactly **one** reconcile + **one** failure `notify` fire when `pendingCount` reaches 0. These are the optimistic-reconcile/unmount cases that **repository calls alone cannot prove**.
