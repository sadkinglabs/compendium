...
export function groupCollection({
  ...
  const g = new Map();   // code -> { code, name, rows:[{card,set,owned,foil,added}] }
  ...
  // axis is active, every printing passes; the finish scope reframes the owned/missing/playset/qty
  // math consistently. Wishlist is judged on the whole card, not the printing.
  ...
  for (const c of (pool || [])) {
    ...
    for (const s of (c._sets || [])) {
      ...
      const row = { card: c, set: s.code, owned: oc?.owned || 0, foil: oc?.foil || 0, added: oc?.added || '' };
      if (!matches(row, isWish)) continue;
  ...
  if (sets.length === 0 || sets.includes(UNCATEGORISED_LABEL)) {
    ...
    for (const [k, v] of owBySet) {
      ...
      if (!card) continue;
      const row = { card, set: UNCATEGORISED_BUCKET, owned, foil, added: v.added || '' };
      if (!matches(row, wishSet.has(card.card_id))) continue;
