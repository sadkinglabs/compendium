...
test('All lens with an owned scope shows the Unspecified pile', () => {
  const g = run({ sets: ['Unspecified'], viewMode: 'all', ownActive: true, ownScope: ['regular'] });
  assert.deepEqual(idsIn(g, ''), ['b']);
});
...
test('a foil-only UNSPECIFIED row is classified the same as a foil-only printed row', () => {
  // The inconsistency this fixes: printed rows used owned > 0 while the Unspecified pile used
  // owned + foil > 0, so the identical physical state was "not owned" in a set and "owned"
  // under Unspecified. Same card, same shelf, two answers.
  ...
  assert.deepEqual(idsIn(asOwned, ''), [], 'foil-only must NOT count as owned under Unspecified');
