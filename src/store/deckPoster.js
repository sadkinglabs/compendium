...
async function _buildDeckPosterCanvas(deck) {
  ...
  const INK = '#0c0716', GOLD = '#dcb86f';
  const ELS = ['air', 'earth', 'fire', 'water'];
  const EL_ORDER = ['Air', 'Earth', 'Fire', 'Water', 'Multi', 'Neutral'];
  const EL_GRAD = { Air: ['#9a90ac', '#5c5470'], Earth: ['#d2974f', '#8a5e2e'], Fire: ['#f07a52', '#a8381f'], Water: ['#5fa8d4', '#2f6f9e'], Multi: ['#e8ca48', '#a8861f'], Neutral: ['#a87fd6', '#6a4a96'] };
  const EL_PIE = { Air: '#c4cdd6', Earth: '#c2864a', Fire: '#e0623f', Water: '#4aa3d4', Multi: '#e3bd45', Neutral: '#a87fd6' };
  ...
  rg.addColorStop(0, '#241636'); rg.addColorStop(0.55, '#150d24'); rg.addColorStop(1, INK);
  ...
  hgrad.addColorStop(0, 'rgba(12,7,22,.10)'); hgrad.addColorStop(0.62, 'rgba(12,7,22,.30)'); hgrad.addColorStop(1, 'rgba(12,7,22,0)');
  ...
  sections.forEach((sec) => {
    ...
    x.fillStyle = '#9a8cae'; x.font = "400 15px 'IBM Plex Mono', monospace"; x.fillText(`${sec.count}`, pad + tw + 12, sec.top + 21);
    ...
    sec.cols.forEach((col, idx) => {
      ...
      for (const l of col) {
        ...
        if (l.k === 'type') { x.fillStyle = '#a87fd6'; x.font = "600 10px 'Hanken Grotesk', sans-serif"; setLS('0.12em'); x.fillText(l.t, cx0, by - 3); setLS('0px'); continue; }
        const m = cardMeta[l.t] || {}, th = m.th || {};
        let rx = cx0 + colW;
        if (sec.cost && m.cost != null) { x.fillStyle = '#c9bfdc'; x.font = "400 13px 'IBM Plex Mono', monospace"; x.textAlign = 'right'; x.fillText(String(m.cost), rx, by); x.textAlign = 'left'; rx -= 20; }
        for (const el of ['water', 'fire', 'earth', 'air']) for (let i = 0; i < (th[el] || 0); i++) { rx -= 15; if (icons[el]) x.drawImage(icons[el], rx, by - 12, 13, 13); }
        x.fillStyle = '#8a7ba6'; x.font = "400 13px 'IBM Plex Mono', monospace"; x.fillText(`${l.q}×`, cx0, by);
        x.fillStyle = '#ece4f5'; x.font = "400 14px 'EB Garamond', Georgia, serif"; const nx = cx0 + 24; x.fillText(fit(l.t, rx - nx - 6), nx, by);
      }
    });
  });
  ...
  (function () {
    ...
    if (!pc.length) { x.fillStyle = '#6e6286'; x.font = "italic 14px 'EB Garamond', Georgia, serif"; x.fillText('Add spells to see the curve.', px, py + 64); return; }
    ...
    const gc = {}; const grad = (el) => { if (gc[el]) return gc[el]; const [c1, c2] = EL_GRAD[el] || ['#a87fd6', '#6a4a96']; const g = x.createLinearGradient(0, PT, 0, PB); g.addColorStop(0, c1); g.addColorStop(1, c2); return gc[el] = g; };
    for (let c = minC; c <= maxC; c++) {
      ...
      for (const el of elements) { const v = costs[c]?.[el] || 0; if (!v) continue; const h = v / niceMax * plotH, segY = yTop - h; x.fillStyle = grad(el);
        if (el === topEl) { x.save(); if (isPeak) { x.shadowColor = 'rgba(157,106,214,.6)'; x.shadowBlur = 14; } topRect(bx, segY, barW, h, Math.min(4, barW / 2, h)); x.fill(); x.restore(); }
        else { x.fillRect(bx, segY, barW, h); } yTop = segY;
      }
      if (totals[c] > 0) { x.fillStyle = isPeak ? GOLD : '#f0e9d8'; x.font = "700 13px 'Cinzel', Georgia, serif"; x.textAlign = 'center'; x.save(); if (isPeak) { x.shadowColor = 'rgba(157,106,214,.55)'; x.shadowBlur = 9; } x.fillText(totals[c], cxx, yTop - 7); x.restore(); x.textAlign = 'left'; }
  ...
  (function () {
    ...
    const drawDonut = (ccx, ccy, slices, total, label) => {
      ...
      else { x.strokeStyle = 'rgba(40,28,60,.6)'; x.beginPath(); x.arc(ccx, ccy, r, 0, 7); x.stroke(); }
      x.strokeStyle = 'rgba(220,184,111,.3)'; x.lineWidth = 1; x.beginPath(); x.arc(ccx, ccy, Ro, 0, 7); x.stroke();
      const ig = x.createRadialGradient(ccx, ccy - Ri * 0.4, 1, ccx, ccy, Ri); ig.addColorStop(0, '#1c1230'); ig.addColorStop(1, '#0c0716');
      ...
      x.fillStyle = '#8a7ba6'; x.font = "600 7.5px 'Hanken Grotesk', sans-serif"; setLS('0.05em'); x.fillText(label, ccx, ccy + 14); setLS('0px'); x.textAlign = 'left';
    };
    ...
    CAT.forEach(([t, label], i) => {
      ...
      x.fillStyle = '#c9bfdc'; x.font = "500 11px 'Hanken Grotesk', sans-serif"; x.fillText(label, barsX, ry + 9);
      x.fillStyle = '#8a7ba6'; x.font = "400 11px 'IBM Plex Mono', monospace"; x.textAlign = 'right'; x.fillText(String(tot), px + pw, ry + 9); x.textAlign = 'left';
      const tY = ry + 14, tH = 7; x.fillStyle = 'rgba(40,28,60,.5)'; rrect(barsX, tY, barsW, tH, 3.5); x.fill();
    ...
    x.fillStyle = '#9a8cae'; x.font = "italic 12px 'EB Garamond', Georgia, serif"; x.fillText('avg spell cost', infoX + avgW + 12, aTop + 27);
    ...
    if (!needs.length) { x.fillStyle = '#7a6e96'; x.font = "italic 12px 'EB Garamond', Georgia, serif"; x.fillText('No thresholds required', infoX, ny); }
    needs.forEach((k) => { if (icons[k]) x.drawImage(icons[k], infoX, ny - 11, 14, 14);
      x.fillStyle = '#9a8cae'; x.font = "400 12px 'Hanken Grotesk', sans-serif"; x.fillText(`need ${peak[k]} · ${sup[k]} sites`, infoX + 20, ny);
    ...
    leg.forEach((k) => { x.fillStyle = EL_PIE[k]; rrect(lx, ly - 9, 10, 10, 3); x.fill(); x.fillStyle = '#9a8cae'; x.fillText(k, lx + 15, ly); lx += 15 + x.measureText(k).width + 14; });
  })();
