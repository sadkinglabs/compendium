...
export const CHANGELOG = [
  {
    ...
    version: '1.2.0-alpha',
    date: '2026-08-31',
    notes: 'Your collection gets places to live, wanted lists that shop for you, and decks that followed Curiosa to its new home.',
    changes: [
      { kind: 'added', text: 'Collection Storage: give your copies physical places - a binder, a bulk box, a cookie tin. Every card starts Unfiled, and staying unfiled is fine; filing is for knowing where things actually are.' },
      { kind: 'added', text: 'Filed copies are protected: the collection refuses to remove copies that are placed somewhere. Un-file them first - unless you remove a card entirely, in which case the filing goes with it.' },
      { kind: 'added', text: 'A card with filed copies wears an In Storage seal on its sheet, beside the playset seal. The seals now have their space reserved, so appearing never nudges the art.' },
      { kind: 'added', text: 'Long-press and drag to rearrange your decks and your storage places.' },
      { kind: 'added', text: 'Wanted lists open split into Missing and Complete sections, and the text export can copy just the cards you are missing - with the quantities you still need, not the ones you set out for.' },
      { kind: 'changed', text: 'Deck import and sync now talk to sorcerytcg.com, where Curiosa decks now live. Saved deck links keep working, and the Maybeboard their new platform exposes is written into the deck notes as its own tidy block.' },
      { kind: 'changed', text: 'Codex collections are now Folios - same feature, clearer name. The FOLIOS section in Marginalia is always on show, and you can name a new one without entering Edit.' },
      { kind: 'changed', text: 'Lists and Storage speak one hierarchy language: pinned things (the Wishlist, Unfiled) sit apart under their own PINNED header, above the sections that are yours to manage.' },
      { kind: 'changed', text: 'The mana and threshold counter is about 10 percent bigger, and Roll for Turn is harder to ignore - engage it or dismiss it, but decide. It now waits in black and white with the rest of the table.' },
      { kind: 'changed', text: 'A deck’s Notes box opens at double height when it holds text.' },
      { kind: 'fixed', text: 'A match ending with both avatars at Death’s Door now records the right winner - the final blow names who fell.' },
      { kind: 'fixed', text: 'Collection screens arrive with their intended entrance instead of popping in when they finish loading.' },
    ],
  },
  {
    build: 274,
    ...
    changes: [
      { kind: 'added', text: 'Advanced counters in a match: an opt-in band on the divider tracks mana and elemental thresholds for both players.' },
      ...
      { kind: 'changed', text: 'The new match avatar picker was redesigned around a three-column grid that makes room for the keyboard as you type.' },
      { kind: 'changed', text: 'Deck text exports order has been reworked to match Curiosa conventions. The on-screen deck keeps its own order.' },
      ...
      { kind: 'changed', text: 'The scanner says what it actually does: point it at a card or a shared QR code. The hint steps out of the way once you have taken your first shot.' },
      { kind: 'changed', text: 'Confirmations now arrive as a centred dialog rather than a second sheet stacked on the first, so an irreversible choice can’t be swiped away by accident.' },
      ...
      { kind: 'fixed', text: 'A “Missing for” list is now an ordinary list, so wanted tracking no longer calls a deck complete when it is a card short.' },
    ],
  },
  {
    build: 199,
