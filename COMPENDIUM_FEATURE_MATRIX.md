...
| Bulk entry | Paste card text, then review a resolved preview before committing: single-set cards auto-file to their set, multi-set (reprinted) cards get a set toggle defaulting to Uncategorised, and unrecognised names are listed and skipped | `owned_cards` | **Implemented** |
