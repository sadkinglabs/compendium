...
| Bulk selection (scoped) | Inside a set: scope the grid (search/refine), "Select all" from the overflow, then the docked search bar becomes an action bar - **Edit copies** (Add or Set an exact count, Standard or Foil; Set to 0 removes while keeping any wishlist want) or **New list** from the selection (then export from Lists). Selection captures each printing at pick time; a card lacking the chosen finish is skipped and reported. Each write is one barrier-guarded transaction with a single broadcast | `owned_cards`, `card_lists`, `ownedImportRepository.js` (`setOwnedItemsBulk`, `createListWithEntries`) | **Implemented** |
