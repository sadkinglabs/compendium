...
| My Collection | A **Sets \| All** toggle over one landing. **Sets**: sets-completion landing (tile per set, non-foil completion + foil count) drilling into a per-set card grid. **All**: a single flat grid of every collector item across every set, one tile per printing. Both surfaces run the SAME refine engine (search + refinement + sort + group) and the SAME selection controller; ALL renders a progressive prefix (first ~100 tiles, grown by a viewport sentinel with an explicit Show-more fallback) so first paint is bounded regardless of catalogue size - presentation only, never the selection or result scope. Both surfaces also carry an **A-Z jump rail**: drag or tap a letter and, on **release** (a latch, so the drag never fights the grid), the list jumps to it - a big letter pill floats beside the thumb during the scrub, with a haptic tick per letter. It shows only when the order is globally alphabetical by name and ducks otherwise; a jump on ALL grows the prefix to the target first | `owned_cards` + shared `cards`, `useCollectionRefine.js`, `collectionAllModel.js`, `alphabetIndex.js`, `AlphabetRail.jsx` | **Implemented** |
...
| List arrange | Group a list by Set (printing-faithful), Rarity, or Element with anchored section headers; sort within groups by name (both directions), rarity, element palette order, or recency; opened from a stacked filter FAB in the refine-sheet language | Session-scoped presentation state (no writes) | **Implemented** |
...
| Native-fidelity sheets | One bottom-sheet chassis app-wide with interruptible spring motion, 1:1 full-sheet drag with scroll handoff, velocity-aware dismissal, rubber banding, a real exit animation, deferred content mount, explicit stacking, and background inert; parameters and gesture state machine per the unified spec | `GothicSheet.jsx`, `sheetMotion.js`, `overlayStack.js`, `docs/bottom-sheet-spec.md` | **Implemented** — Android values shipping; iOS column specified, not implemented (owner-deferred) |
