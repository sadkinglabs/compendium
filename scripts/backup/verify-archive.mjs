// Verify a real Compendium archive by restoring it into a database that has NEVER held it.
//
...
//   3. restores it into a fresh schema-v11 database seeded with the real catalog, so v11
//      canonicalisation resolves printings exactly as it does on device
...
const { previewBackup, restoreAll } = await import(B+'backupService.js');
...
// A FRESH install: one auto-created starter, exactly like first boot.
...
const t0 = Date.now();
const res = await restoreAll(preview);
...
for (const unit of preview.env.payload.profiles) {
  const pid = rows("SELECT id FROM profiles WHERE name LIKE ?||'%' AND id!='starter' ORDER BY created_at DESC;", [unit.profile.name])[0]?.id;
...
console.log('starter still present  :', rows("SELECT COUNT(*) c FROM profiles WHERE id='starter';")[0].c === 1);
