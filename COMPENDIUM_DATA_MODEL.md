...
`variants` holds one entry per physical printing: `{ slug, set, setName, finish, product, artist, flavorText, image }`. `image` is the per-printing art KEY (a content-addressed `<slug>.<sha256>.webp` name), or `null` for a printing with no art. The key names the art, never a URL; the art boundary (`useArtSource`/`ArtImage`) resolves it through the on-device cache, then the Cloudflare R2 CDN, then a deterministic element-gradient placeholder (art-CDN Phase 5 removed the bundled fallback), and honours zero-image mode - so offline-first holds via the on-device cache + the placeholder, while a fresh install with no network shows placeholders for card art until it has been online once. Only the catalog JSON and the manifest bundle with the app; the card bytes are CDN-served. `sets` is the card-level distinct-set list, each `{ name, code }`. Set codes: `001` Alpha, `002` Beta, `004` Arthurian Legends, `005` Dragonlord, `006` Gothic, `999` Promotional (there is no `003`). **Set metadata is data-derived, not hardcoded:** the pipeline writes the code-to-name map to `src/store/setCatalog.json`, and `src/store/sets.js` reads those names while deriving display order from the numeric code (sequential by design). A new set therefore needs no source edit - its name arrives with its cards and its order slots in by code.
...
The shared catalog is bundled reference data that is reseeded whole when its content changes. `src/store/catalogVersion.json` is a generated `{ version, hash }` token: `version` is a human-facing integer (currently **6**), and `hash` is a SHA-256 over the exact serialised generation (cards, articles, FAQs, link graph, the sorted per-printing **(slug, art-key) projection** - not the full manifest, so an encoder-only metadata change that keeps the bytes does not bump the version - and the compiled Codex build hash). `src/store/catalog.js` imports it as `CATALOG_VERSION`.
