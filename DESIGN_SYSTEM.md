...
**`scanner/ui/CameraOverlay.kt`** + `ScannerScreen.kt` (Compose, native `ScannerActivity` — NOT the WebView) — recognition as ONE staged act on a **single shared clock** (`ScannerScreen` owns a `reveal` 0→1, shared by the overlay AND the result tray so they never drift apart). Luxe reads from **material depth**, not motion quantity — four layers of one silhouette: an `#17130B` **ink shadow** debossed under the gold (+1dp offset), the **gold body** (double rule: ~2.25dp outer at +6dp outset, 1dp inner on the guide), a wide low-alpha **halo** (no blur), and a thin `#EFE7D8` **sheen** skimming the top (−0.5dp); four gold **corner bosses** (studs on ink dots) pin it. The physical card stays visible throughout; **nothing is ever drawn over the artwork and no remote image is involved**.
**Three states (feedback loop is load-bearing — recognition must be *guided* and must *culminate*, never a single flat jump):**
- **ready** — still parchment `#EFE7D8` corner guides (rounded to the card's own radius) over a ~50% dimmed surround; "Position a card within the frame." No idle animation.
- **recognising** — a **full breathing violet frame** (55–75% over 1.2s) + violet corners: the clear "engaged and reading" signal; "Hold steady". A single light engagement tick fires on ENTERING this state, not repeatedly.
- **recognised** — as `reveal` runs: the violet guide extinguishes and the gilt double-rule makes **firm contact via brightness + stroke weight (NO frame scaling)**; a **strong, short gold flare** peaks (~50% alpha) synced to the haptic peak, then settles; the card **name TRANSLATES up 14dp and fades in** (no scaling); settled and motionless by ~800ms. Coloured by the **result's type accent token** (gold card / violet deck / jade match via `accentFor()`).
