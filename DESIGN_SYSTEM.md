...
- **Header overflow** `[Shipping]` — `OverflowMenu`: a dots trigger in a header (34px visual circle inside a >=44px hit box, per the §5 touch floor), opening a small anchored menu of **screen-level commands that are not the primary action**. Rename, duplicate, export, delete, import-from-text. First consumer: Collection › Overview (typed import, once the FAB became camera-only).
