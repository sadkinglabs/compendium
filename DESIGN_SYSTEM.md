...
- **Waiting** `[Shipping]` — two tiers, chosen by how much of the screen is missing. A whole pillar arriving gets **`PillarLoading`**: the brand diamond (shared geometry, `brandMark.js`) breathing centred on the dark ground, so the wait is branded and deliberate rather than a stray ellipsis on an empty frame. It **waits 140ms before fading in**, so a warm chunk resolves invisibly and the loader never strobes on an ordinary tab press; the breath is **opacity + transform only** (an infinite paint-phase animation on every pillar change would raster the region every frame — §6), with a still mark under reduced motion. An in-place gap *inside* an already-drawn surface (a sheet body, a list section) keeps the quiet **`Loading`** ellipsis — the screen is present, only a part of it is pending.
