...
- **Sheets** `[Shipping]` — one chassis (`GothicSheet`), rebuilt to the unified spec ([`docs/bottom-sheet-spec.md`](docs/bottom-sheet-spec.md), 2026-08-15): one interruptible rAF spring (stiffness 235 / damping 31, ~300ms, Compose parity) drives open, close and snap-back; full-sheet drag with the scroll handoff state machine; velocity-projected dismissal (`y + v×0.1s > 0.25H`, flick ≥500px/s); iOS rubber band (c=0.55) past the top; position-linked scrim while opening/dragging that FREEZES on an accepted dismissal so close reads as a pure slide (panel off-screen first, then a 100ms scrim fade - owner ruling); a REAL exit animation (the chassis stays mounted through close); the scrim catches a still-moving sheet so an entrance is grabbable, while staying tap-only once open; **the panel's compositor layer stays promoted for its whole mounted life** (`will-change: transform` never removed, settles at a written `translate3d(0, 0%, 0)`, never a cleared transform) - demoting it at settle produced a partial-raster glitch at full height on-device, so this deliberately overrides the generic "transient hints" guidance (§6 WebView paint family; `docs/bottom-sheet-spec.md` constraint 7); background `inert` via `overlayStack.js`; the handle is a screen-reader "Close" button; Escape rides the back-consumer stack. Chrome is tokenized: `--surface-sheet` (#100c08 flat, owner-ruled), `--radius-sheet` 30px, `--shadow-sheet`, `--gold-handle`. `Sheet` is the one titled adapter (`BottomSheet` is an alias; titles are pinned headers everywhere). **Hard WebView paint rule (§6):** a fixed scrim carries opacity-only animation; a `position:relative` panel carries the transform; any nested scroller gets its own opaque `translateZ(0)` layer.
