...
- **Header overflow** `[Shipping]` — `OverflowMenu`: a 44px dots trigger in a header (one circle at the §5 touch floor - an inner 34px styling span cost the button its accessible name in the WebView, so the button IS the circle), opening a small anchored menu of **screen-level commands that are not the primary action**. It shares the FAB-menu chassis and a common `MenuGlyph` set so glyph sizing is consistent. **Manage-only**: select, edit, delete, and bulk-add — never adding or importing.
  - **Boundary against the FAB, stated as a rule.** Every way to *add cards into the collection* lives on a single `+` **FAB menu** per surface (typed import, camera, and on a list, export) — scoped to where you are, each option labelled so nothing fires unannounced. A second FAB carries filter/sort where a surface needs it. The **overflow is manage-only**: select, rename, duplicate, delete, and bulk edit. A screen may carry both a `+` FAB and an overflow — the set drill has a refine FAB, a `+` FAB and a Set-actions overflow — so they are deliberately separate components rather than one with two anchors. (History: adding once lived in the overflow as typed-import; it moved onto the `+` FAB so the camera is never buried.)
  - **Every screen with manage or bulk commands gets one.** Overview (select · rename · delete), set drill (select all → bulk edit / new list), list detail (rename · duplicate · delete). Consistency here is what makes the FAB's meaning learnable: adding is always the `+` FAB, managing is always the overflow.
