...
- **My Collection** opens on a sets-completion landing (one tile per set, non-foil completion); a tile drills into that set's cards as a card grid with permanent steppers - adding is a place, not a mode. Per-set quantities (Alpha and Beta tracked separately), filters, bulk entry, and scanner-assisted entry.
...
│   │   ├─ owned_cards[]       # one row per collector item (cardId + set + finish); owned and wanted quantities
...
`owned_cards` is the Collection pillar's profile-scoped ownership ledger. Its grain is the **collector item** - card, set, and finish - so Alpha, Alpha foil, Beta, and Beta foil are four distinct rows, and a want is a property of one of them rather than of the card (see the key space in `COMPENDIUM_DATA_MODEL.md`). `card_lists` contains custom groupings and wanted-card goals. Neither is the deck `collection` zone, and neither is a Codex named collection; those are separate domain concepts with separate persistence contracts. The concrete relational schema is defined in `COMPENDIUM_DATA_MODEL.md`.
...
**Collection pure models.** The Collection pillar's consequential non-write logic lives in pure, DOM-free store modules tested under `test:query`, kept out of the ~1,400-line `Collection.jsx`: `collectionGroups.js` (printing grouping + ownership lens), `importPlan.js` (text-import printing routing — single/multi/Uncategorised partition, choice defaults, write-item assembly), and `listGoalModel.js` (wishlist/list completion math — the optimistic `goalTotals` and per-row `goalRowState` behind every progress bar). Optimistic ownership now shares ONE contract: `ownedStepController.js` (provisional/confirmed — displayed = confirmed + pending, reconcile only when a row's write chain drains, a failed authoritative read never counts as confirmation, visible failure recovery). `useOwnedLedger` binds it for the card sheet, and `ownedStepGrid.js` binds it for the set grid, creating a controller lazily per *tapped* row so a ~780-tile set costs nothing until edited. The `ListDetail` goal machine stays separate (it edits list targets, not the ownership ledger) and reconciles via `collectionGoalDrain.js`.
...
- **The catalog is regenerated by a build-time tool, never at runtime.** `npm run update:catalog` (`scripts/update-catalog.mjs` + `scripts/catalog/*`) rebuilds the entire bundled catalog from a `CATALOG_DROP/` folder - Curiosa tRPC fetch and merge, rules/FAQ CSV compile, per-printing PNG-to-WebP conversion, link-graph and compiled-Codex regeneration - through a staged, validated, journaled promotion, and writes the `src/store/catalogVersion.json` seed token that triggers the on-device reseed. It runs on a developer machine only; the app never fetches or mutates catalog content at runtime. A content-hash gate makes an unchanged catalog a no-op, and a build-wide guard refuses to package a working tree with a promotion mid-flight. See `BUILD.md` and `COMPENDIUM_DATA_MODEL.md`.
  - **Art-CDN migration (in progress):** the same command additionally converts each scan to the content-addressed CDN tier, atomically stages it under `CATALOG_DROP/cdn-art/`, and writes a prospective `art-manifest.json` to `.catalog-build/`. During the migration its catalog **promote is dormant** - no `public/`/`src/` repoint - and objects are published additively to R2 by `scripts/catalog/cdn-upload.mjs` (create-only conditional PUTs, audited). Promotion returns as the single atomic Phase-2 activation. See `docs/proposals/art-cdn-migration.md`.
...
- **What:** Overview, My Collection, and Lists surfaces. My Collection is a sets-completion landing drilling into a per-set card grid, with per-set owned quantities (opened from inside a set, the card sheet is locked to that printing), a wishlist toggle, filters, bulk import, and camera-assisted entry. Tapping card art raises a full-screen viewer with gyro parallax. Lists provide named card and wanted lists with progress, in three distinct grammars (wishlist heart · goal bar · fanned thumbs). Deck buildability compares owned cards with deck requirements without reserving cards.
